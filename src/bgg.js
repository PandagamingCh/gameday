...
async function syncUserCollection(userId, bggUsername) {
  ...
  await db.transaction(async (client) => {
    await client.query('DELETE FROM bgg_games WHERE user_id = $1', [userId]);
    for (const g of games) {
      await client.query(`
        ...
        ON CONFLICT (user_id, bgg_id) DO UPDATE SET
          name = EXCLUDED.name, year = EXCLUDED.year, thumbnail = EXCLUDED.thumbnail,
          min_players = EXCLUDED.min_players, max_players = EXCLUDED.max_players,
          min_time = EXCLUDED.min_time, max_time = EXCLUDED.max_time,
          bgg_rating = EXCLUDED.bgg_rating, bgg_weight = EXCLUDED.bgg_weight
      `, [userId, g.bggId, g.name, g.year, g.thumbnail,
          g.minPlayers, g.maxPlayers, g.minTime, g.maxTime,
          ratingMap[g.bggId]?.rating || null, ratingMap[g.bggId]?.weight || null]);
    }
    await client.query("UPDATE users SET bgg_synced_at = to_char(NOW(), 'YYYY-MM-DD HH24:MI:SS') WHERE id = $1", [userId]);
  });
