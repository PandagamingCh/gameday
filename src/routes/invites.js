...
router.post('/api/invites', requireAdmin, async (req, res) => {
  const token = uuidv4();
  await db.run('INSERT INTO invites (token, created_by) VALUES (?, ?)', [token, req.session.userId]);
...
router.delete('/api/invites/:token', requireAdmin, async (req, res) => {
  await db.run('UPDATE invites SET is_active = 0 WHERE token = ?', [req.params.token]);
  res.json({ ok: true });
});
