...
router.post('/api/bgg/sync', requireAuth, async (req, res) => {
  const user = await db.get('SELECT bgg_username FROM users WHERE id = ?', [req.session.userId]);
...
router.post('/api/sessions/:id/bgg/enrich', requireAdmin, async (req, res) => {
  ...
  const props = await db.all("SELECT DISTINCT bgg_id FROM proposals WHERE session_id=? AND bgg_id != '' AND bgg_id IS NOT NULL", [sessionId]);
  ...
  for (const p of props) {
    try {
      ...
      if (data.bgg_rating || data.bgg_weight) {
        await db.run('UPDATE proposals SET bgg_rating=COALESCE($1,bgg_rating), bgg_weight=COALESCE($2,bgg_weight) WHERE session_id=$3 AND bgg_id=$4', [data.bgg_rating || null, data.bgg_weight || null, sessionId, p.bgg_id]);
        updated++;
      }
...
router.get('/api/bgg/collection/:userId', requireAuth, async (req, res) => {
  const games = await getUserCollection(parseInt(req.params.userId));
  res.json({ games });
});
