...
router.patch('/api/settings', requireAdmin, async (req, res) => {
  const { key, value } = req.body;
  if (!key) return res.status(400).json({ error: 'key requis' });
  await db.run(`INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value`, [key, JSON.stringify(value)]);
  res.json({ ok: true });
});
...
router.post('/api/auth/forgot-password', async (req, res) => {
  ...
  const user = await db.get(`SELECT id, username FROM users WHERE email = $1`, [email.toLowerCase().trim()]);
  ...
  await db.run('INSERT INTO reset_tokens (token, user_id, expires_at) VALUES (?, ?, ?)', [token, user.id, expires]);
...
router.post('/api/auth/reset-password', async (req, res) => {
  ...
  const row = await db.get('SELECT * FROM reset_tokens WHERE token = ? AND used = 0', [token]);
...
router.patch('/api/profile/email', requireAuth, async (req, res) => {
  const { email } = req.body;
  const emailVal = email ? email.toLowerCase().trim() : null;
  if (emailVal) {
    const existing = await db.get('SELECT id FROM users WHERE email = ? AND id != ?', [emailVal, req.session.userId]);
    if (existing) return res.status(400).json({ error: 'Cet email est déjà utilisé' });
  }
  await db.run('UPDATE users SET email = ? WHERE id = ?', [emailVal, req.session.userId]);
  res.json({ ok: true });
});

// GET /api/permissions
router.get('/api/permissions', requireAuth, async (req, res) => {
  const perms = await db.all('SELECT action, level FROM permissions');
  res.json({ permissions: perms });
});

// PATCH /api/permissions — admin only
router.patch('/api/permissions', requireAdmin, async (req, res) => {
  ...
  await db.transaction(async (client) => {
    for (const { action, level } of permissions) {
      await client.query('INSERT INTO permissions (action, level) VALUES ($1,$2) ON CONFLICT (action) DO UPDATE SET level=EXCLUDED.level', [action, parseInt(level)]);
    }
  });
  res.json({ ok: true });
});
